...
BindRequestMsg = b'0001'
BindResponseMsg = b'0101'
BindErrorResponseMsg = b'0111'
SharedSecretRequestMsg = b'0002'
SharedSecretResponseMsg = b'0102'
SharedSecretErrorResponseMsg = b'0112'
...
def stun_test(sock, host, port, source_ip, source_port, send_data=""):
    ...
    tranid = gen_tran_id()
    str_data = ''.join([BindRequestMsg.decode("utf-8"), str_len, tranid, send_data])
...
def get_nat_type(s, source_ip, source_port, stun_host=None, stun_port=3478):
    ...
    if ret['ExternalIP'] == source_ip:
        changeRequest = ''.join([ChangeRequest.decode("utf-8"), '0004', "00000006"])
        ret = stun_test(s, stun_host, port, source_ip, source_port,
                        changeRequest)
        if ret['Resp']:
            typ = OpenInternet
        ...
        if ret['Resp']:
            ...
            if not ret['Resp']:
                ...
                if exIP == ret['ExternalIP'] and exPort == ret['ExternalPort']:
                    changePortRequest = ''.join([ChangeRequest.decode("utf-8"), '0004', "00000002"])
                    log.debug("Do Test3")
